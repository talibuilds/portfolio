...
export default function SunEarthBackground() {
  ...
  useEffect(() => {
    ...
    mount.innerHTML = ''; // Clear any existing canvases from HMR
    ...
    sunGroup.position.set(4.5, 0.6, 0); // Positioned nicely on the right
    scene.add(sunGroup);
    ...
    // Removed corona and halo to avoid extra yellow rings
    ...
    // Removed atmo layer to avoid extra blue ring
    ...
    let camAngle = 0.6, camElev = 0.35, camDist = 12.7; // Zoomed out 10% more
    let dragging = false, lastX = 0, lastY = 0;

    let cameraTargetX = 1.0;

    function updateCamera() {
      const x = camDist * Math.cos(camElev) * Math.sin(camAngle) + cameraTargetX;
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", background: "#000", zIndex: 1 }}>
